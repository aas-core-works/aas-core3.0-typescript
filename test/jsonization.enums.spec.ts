...
test("ModellingKind round-trip OK", () => {
  ...
  const literalOrError = AasJsonization.modellingKindFromJsonable(jsonable);

  expect(literalOrError.error).toBeNull();
  const literal = literalOrError.mustValue();

  expect(literal).toStrictEqual(AasTypes.ModellingKind.Template);
});
...
test("ReferenceTypes round-trip OK", () => {
  ...
  expect(literal).toStrictEqual(AasTypes.ReferenceTypes.ExternalReference);
});
...
test("KeyTypes round-trip OK", () => {
  ...
  expect(literal).toStrictEqual(AasTypes.KeyTypes.AnnotatedRelationshipElement);
});
